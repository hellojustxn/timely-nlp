...
# Generate a data.json file containing a list of grouped events, each group events can have one or more events
def main():
  if len(sys.argv) != 2:
  ...
  for i in range(NUM_EVENTS):
    group = []
    # TODO: Include more event names in eventNames.py
    # Generate a random start time and duration
    for i in range(random.randint(1, MAX_EVENTS_IN_GROUP)):
